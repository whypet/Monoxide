...
{
	INT nSampleCount = ( nDuration / 10 ) * 441;
	HANDLE hHeap = GetProcessHeap( );
	PSHORT psSamples = HeapAlloc( hHeap, HEAP_ZERO_MEMORY, nSampleCount * sizeof( SHORT ) );

	switch ( nWaveformIndex )
	{
	case 0:
		for ( INT i = 0; i < nSampleCount; i++ )
			psSamples[ i ] = ( SHORT )( SineWave( i, psFreq[ i ], 44100 ) * ( ( FLOAT )SHRT_MAX * pfVolume[ i ] ) );
		break;
	case 1:
		for ( INT i = 0; i < nSampleCount; i++ )
			psSamples[ i ] = ( SHORT )( SquareWave( i, psFreq[ i ], 44100 ) * ( ( FLOAT )SHRT_MAX * pfVolume[ i ] ) );
		break;
	case 2:
		for ( INT i = 0; i < nSampleCount; i++ )
			psSamples[ i ] = ( SHORT )( TriangleWave( i, psFreq[ i ], 44100 ) * ( ( FLOAT )SHRT_MAX * pfVolume[ i ] ) );
		break;
	case 3:
		for ( INT i = 0; i < nSampleCount; i++ )
			psSamples[ i ] = ( SHORT )( SawtoothWave( i, psFreq[ i ], 44100 ) * ( ( FLOAT )SHRT_MAX * pfVolume[ i ] ) );
		break;
	case 4:
		for ( INT i = 0; i < nSampleCount; i++ )
		{
			FLOAT f = ( ( FLOAT )fabs( SineWave( i, psFreq[ i ], 44100 ) ) * 2.f ) - 1.f;
			psSamples[ i ] = ( SHORT )( f * ( ( FLOAT )SHRT_MAX * pfVolume[ i ] ) );
		}
		break;
	case 5:
		for ( INT i = 0, j = 0; i < nSampleCount; i++ )
		{
			FLOAT f = ( -( FLOAT )fabs( SineWave( ( j % 11 ) > 5 ? j += 4 : j, psFreq[ i ], 44100 ) ) * 2.f ) + 1.f;
			psSamples[ i ] = ( SHORT )( f * ( ( FLOAT )SHRT_MAX * pfVolume[ i ] ) );
			j++;
		}
		break;
	case 6:
		for ( INT i = 0; i < nSampleCount; i++ )
		{
			FLOAT f = ( -( FLOAT )fabs( SineWave( i + ( FastCosine( ( FLOAT )i ) * 50.f ), psFreq[ i ], 44100 ) ) * 2.f ) + 1.f;
			psSamples[ i ] = ( SHORT )( f * ( ( FLOAT )SHRT_MAX * pfVolume[ i ] ) );
		}
		break;
	case 7:
		for ( INT i = 0; i < nSampleCount; i++ )
		{
			FLOAT f = ( -( FLOAT )fabs( SineWave( i + ( FastSine( ( FLOAT )i ) * 500.f ), psFreq[ i ], 44100 ) ) * 2.f ) + 1.f;
			psSamples[ i ] = ( SHORT )( f * ( ( FLOAT )SHRT_MAX * pfVolume[ i ] ) );
		}
		break;
	}

	WAVEFORMATEX waveFormat = { WAVE_FORMAT_PCM, 1, 44100, 44100 * 2, 2, 16, 0 };
	HWAVEOUT hWaveOut;
	waveOutOpen( &hWaveOut, WAVE_MAPPER, &waveFormat, 0, 0, CALLBACK_NULL );

	WAVEHDR waveHdr = { ( CHAR * )psSamples, nSampleCount * sizeof( SHORT ), 0, 0, 0, 0, NULL, 0 };
